...
from wtforms import (BooleanField, PasswordField, StringField, SubmitField, TextAreaField,
                     IntegerField, MultipleFileField, URLField, FileField)
from wtforms.validators import DataRequired, Email, EqualTo, NumberRange, Length, ValidationError, URL
...
class FeedbackForm(FlaskForm):
    ...
    raiting = IntegerField('Rating', validators=[DataRequired(), NumberRange(min=0, max=10)],
                           render_kw={"class": "form-control"})
...
class ApartmentForm(FlaskForm):
    owner_name = StringField('Введите имя арендатора', validators=[DataRequired()],
                               render_kw={"class": "form-control"})
    city = StringField('Город', validators=[DataRequired()], render_kw={"class": "form-control"})
    area = StringField('Район', validators=[DataRequired()], render_kw={"class": "form-control"})
    address = StringField('Улица', validators=[DataRequired()], render_kw={"class": "form-control"})
    bld = IntegerField('Дом', validators=[DataRequired()], render_kw={"class": "form-control"})
    apt = IntegerField('Квартира', validators=[DataRequired()], render_kw={"class": "form-control"})
    rooms = IntegerField('Количество комнат', validators=[DataRequired()], render_kw={"class": "form-control"})
    price = IntegerField('Price', validators=[DataRequired()], render_kw={"class": "form-control"})
    url = URLField('Введите ссылку', render_kw={"class": "form-control"})
    photo = FileField('Загрузите фото', render_kw={"class": "form-control-file"})
    submit = SubmitField('Создать квартиру', render_kw={"class": "btn btn-lg btn-primary btn-block btn-outline-*"})
